...
nodos_requeridos = set()
for (i, j) in arcos_req:
    nodos_requeridos.add(i)
    nodos_requeridos.add(j)

model.addConstrs((
    gp.quicksum(x[i, v] for (i, vv) in arcos_req if vv == v) +
    gp.quicksum(x[v, j] for (vv, j) in arcos_req if vv == v) >= 1
    for v in nodos_requeridos
), name="VisitarTodosLosNodosRelevantes")
...
try:
    ...
    with open(multiplicidad_path, 'w') as f_mul:
        f_mul.write("Arco_i Arco_j VecesRecorrido Tipo\n")
        f_mul.write("=================================\n")

        # 1) Unidireccionales: se reportan tal cual vienen del solver
        for (i, j) in sorted(arcos_req_unidireccionales):
            c = x_multiplicidad.get((i, j), 0)
            f_mul.write(f"{i} {j} {c} uni\n")

        # 2) Bidireccionales: aplicar colapso por par con max
        #    Si total>0, se escribe EN AMBAS direcciones con ese total.
        pares_procesados = set()
        for ((i, j), (k, m)) in arcos_req_bidireccionales:
            # normaliza el identificador del par para no duplicar
            par_id = tuple(sorted([(i, j), (k, m)]))
            if par_id in pares_procesados:
                continue
            pares_procesados.add(par_id)

            c_ij = x_multiplicidad.get((i, j), 0)
            c_ji = x_multiplicidad.get((k, m), 0)
            total = max(c_ij, c_ji)

            if total > 0:
                # reportar ambas direcciones como recorridas 'total' veces
                f_mul.write(f"{i} {j} {total} bi\n")
                f_mul.write(f"{k} {m} {total} bi\n")
            else:
                # si prefieres NO listar pares no usados, comenta estas dos líneas
                f_mul.write(f"{i} {j} 0 bi\n")
                f_mul.write(f"{k} {m} 0 bi\n")
